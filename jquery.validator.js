...
(function($, window, undefined) {
    ...
    var helperMethods = {
        ...
        isAlpha: function(str) {
            var preg = /^[^\s][A-z\s\.\-ÆÐƎƏƐƔĲŊŒẞÞǷȜæðǝəɛɣĳŋœĸſßþƿȝĄƁÇĐƊĘĦĮƘŁØƠŞȘŢȚŦŲƯY̨Ƴąɓçđɗęħįƙłøơşșţțŧųưy̨ƴÁÀÂÄǍĂĀÃÅǺĄÆǼǢƁĆĊĈČÇĎḌĐƊÐÉÈĖÊËĚĔĒĘẸƎƏƐĠĜǦĞĢƔáàâäǎăāãåǻąæǽǣɓćċĉčçďḍđɗðéèėêëěĕēęẹǝəɛġĝǧğģɣĤḤĦIÍÌİÎÏǏĬĪĨĮỊĲĴĶƘĹĻŁĽĿʼNŃN̈ŇÑŅŊÓÒÔÖǑŎŌÕŐỌØǾƠŒĥḥħıíìiîïǐĭīĩįịĳĵķƙĸĺļłľŀŉńn̈ňñņŋóòôöǒŏōõőọøǿơœŔŘŖŚŜŠŞȘṢẞŤŢṬŦÞÚÙÛÜǓŬŪŨŰŮŲỤƯẂẀŴẄǷÝỲŶŸȲỸƳŹŻŽẒŕřŗſśŝšşșṣßťţṭŧþúùûüǔŭūũűůųụưẃẁŵẅƿýỳŷÿȳỹƴźżžẓ]+$/i;
            return preg.test(str);
        },
        isIndex: function(str) {
            var preg = [/^0?[0-9]{5}$/, /^[A-z]{2}-?[0-9]{4,5}$/];
            return preg[0].test(str) || preg[1].test(str);
        },
    ...
    var validatorMessages = {
        pl: {
            ...
            checkIsNumber: 'Pole wymaga podania tylko liczb.',
            checkIsAlpha: 'Pole wymaga podania tylko liter.',
            checkIndex: 'Numer artykułu jest niepoprawny.',
            ...
            equalsTo: 'Pole ma inną wartość niż pole "{2}"'
        }
    }
    ...
    var validationMethods = {
        ...
        checkIsAlpha: function(validator) {
            if (helperMethods.removeMaskChars(validator.$element.val()).length == 0) {
                return helperMethods.validationSuccess('checkIsAlpha', validator);
            }
            if (!helperMethods.isAlpha(helperMethods.removeMaskChars(validator.$element.val()))) {
                return helperMethods.validationFailed('checkIsAlpha', validator);
            }
            return helperMethods.validationSuccess('checkIsAlpha', validator);
        },
        checkIndex: function(validator) {
            if (helperMethods.removeMaskChars(validator.$element.val()).length == 0) {
                return helperMethods.validationSuccess('checkIndex', validator);
            }
            if (!helperMethods.isIndex(helperMethods.removeMaskChars(validator.$element.val()))) {
                return helperMethods.validationFailed('checkIndex', validator);
            }
            return helperMethods.validationSuccess('checkIndex', validator);
        },
        ...
        checkPhone: function(validator) {
            ...
            var preg = /^(0|0[0-9]{2}|[0-9]{2}|0048|\+48)?(\s|(-)|(\/)|(\\))*[0-9(\-)(\s)(\\)(\/)]{7,11}$/;
        ...
        checkRegon: function(validator) {
            ...
            var value = helperMethods.removeMaskChars(validator.$element.val());
            if ((value.length !== 9 && value.length !== 14) || !helperMethods.isNumber(value)) {
                return helperMethods.validationFailed('checkRegon', validator);
            }
            if (value.length === 9) {
                var arrSteps9 = new Array(8, 9, 2, 3, 4, 5, 6, 7);
                var intSum = 0;
                for (var i = 0; i < 8; i++) {
                    intSum += arrSteps9[i] * value[i];
                }
                var intFinal = intSum % 11;
                var intControlNr = (intFinal == 10) ? 0 : intFinal;
                ...
                var arrSteps14 = new Array(2, 4, 8, 5, 0, 9, 7, 3, 6, 1, 2, 4, 8);
                var intSum = 0;
                for (var i = 0; i < 13; i++) {
                    intSum += arrSteps14[i] * value[i];
                }
                var intFinal = intSum % 11;
                var intControlNr = (intFinal == 10) ? 0 : intFinal;
